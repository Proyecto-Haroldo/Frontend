...
import { IQuestionnaireAnswer, IQuestionnaireResult } from '../../core/types/questionnaire';
...
function QuestionnaireResults() {
    ...
    const parsedQuestionnaireFromLS = useMemo(() => {
        if (!questionnaireDataFromLS) return null;
        try {
            const parsed = JSON.parse(questionnaireDataFromLS) as IQuestionnaireResult | null;
            return parsed ?? null;
        } catch {
            return null;
        }
    }, [questionnaireDataFromLS]);
    ...
    if (aiRecommendationFromLS) {
        ...
        analysisData = {
            ...
            questionnaire: parsedQuestionnaireFromLS?.metadata?.title || 'Sin Determinar',
            timestamp: parsedQuestionnaireFromLS?.metadata?.timestamp || new Date().toISOString(),
            categoria: parsedQuestionnaireFromLS?.metadata?.category || 'General',
    ...
    const displayAnswers =
        parsedQuestionnaireFromLS?.answers?.map((a: IQuestionnaireAnswer) => ({
